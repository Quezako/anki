...
def on_show_options(menu, deck_id) -> None:
    ...
    child_dyn_ids = _dyn_subdeck_ids(deck_id)
    if child_dyn_ids:
        action = menu.addAction("Empty filtered subdecks")
        action.triggered.connect(lambda _, ids=child_dyn_ids: deckbrowser_empty_subtree_and_refresh(ids))
        action = menu.addAction("Rebuild filtered subdecks")
        action.triggered.connect(lambda _, ids=child_dyn_ids: deckbrowser_rebuild_subtree_and_refresh(ids))


def _dyn_subdeck_ids(deck_id):
    parent = mw.col.decks.get(int(deck_id))
    if not parent:
        return []
    prefix = parent["name"] + "::"
    dyn_ids = []
    for d in mw.col.decks.all():
        if d["dyn"] and d["name"].startswith(prefix):
            if not this_name_excluded_by_config(d["name"]):
                dyn_ids.append(d["id"])
    return sorted(dyn_ids)


def deckbrowser_empty_subtree_and_refresh(dyn_ids):
    for did in dyn_ids:
        mw.col.sched.emptyDyn(did)
    mw.deckBrowser.refresh()


def deckbrowser_rebuild_subtree_and_refresh(dyn_ids):
    for did in dyn_ids:
