...
foreach ($res as $row) {
    ...
    foreach ($arrTags as $tag) {
        if (preg_match('/JLPT::([0-9])/', $tag, $matchJLPT, PREG_OFFSET_CAPTURE)) {
            ...
            $strMnN .= " <n0>{$tag}</n0>";
        } elseif (preg_match('/MnN::([0-9]-[0-9])(::([0-9]+))?/', $tag, $matchMnN, PREG_OFFSET_CAPTURE)) {
            $arrMnN = [
                '1-1' => 5,
                '1-2' => 4,
                '2-1' => 3,
    ...
    if ($iKanji === 0) $iKanji = 1;
    if ($strJLPTK === '') $strJLPTK = ' <n0>JLPT::K1::0</n0>';
    ...
    if ($row[$col['kun_pre']] != '') $strHTML .= "<span class='furigana2'>{$row[$col['kun_pre']]}</span>";
    $strHTML .= implode('', $arrFuri);
    if ($row[$col['kun_post']] != '') $strHTML .= "<span class='furigana2'>{$row[$col['kun_post']]}</span>";
    if ($row[$col['voc_alts']] != '') $strHTML .= " / Alts: {$row[$col['voc_alts']]}<br/>";
    $strHTML .= "</span></span>";

    $strHTML .= "<span class='hover'>FR<span class='french'>: {$row[$col['mean']]}</span></span>";
    if ($row[$col['fr_notes']] != '') $strHTML .= "<span class='hover'>Notes<span class='french'>: {$row[$col['fr_notes']]}</span></span><br/>";
    $strHTML .=  "<hr/>";
}


if (count($res) === 0) {
    echo "No Jukugo found with this kanji.<br>";
    //die;
	$count = 0;
	$intJLPT = 0;
	$intJLPTKFull = 0;
...
<span style='color:rgb(" . ($intJouYouPercent) . "%," . (100 - $intJouYouPercent) . "%, 0%);'>" . $intJouYouFull . "</span> <hr/>";
echo $strHTML;
?>
</div>